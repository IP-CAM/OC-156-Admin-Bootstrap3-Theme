          ...
          <tbody>
            ...
            <tr>
              ...
              <td class="text-right"><div class="btn-group btn-group-xs"><?php foreach ($extension['action'] as $action) { ?>
                <a href="<?php echo $action['href']; ?>" data-toggle="tooltip" title="<?php echo $action['text']; ?>" class="btn <?php $text_uninstall = $this->language->get('text_uninstall'); if ( isset($text_uninstall) && $text_uninstall == $action['text'] ){ echo " btn-default"; $icon="fa fa-times"; }else{ echo " btn-primary"; $icon = "fa fa-edit"; } ?>"><i class="<?php echo $icon; ?>"></i> <?php echo $action['text']; ?></a>
                <?php } ?></div></td>
            </tr>
            <?php } ?>
            <?php } else { ?>
